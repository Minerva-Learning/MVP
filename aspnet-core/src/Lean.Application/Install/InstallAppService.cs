...
using Lean.Configuration.Host.Dto;
using Lean.EntityFrameworkCore;
using Lean.Identity;
using Lean.Install.Dto;
using Lean.Migrations.Seed;
using Lean.Migrations.Seed.Host;
...
namespace Lean.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : LeanAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<LeanDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{LeanConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{LeanConsts.ConnectionStringName}", constring);
        }
